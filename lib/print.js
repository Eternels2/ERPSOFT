...
route('GET', '/print/shipment/:id', async (ctx) => {
  ...
  // Ligne declaree indisponible au picking : seule la quantite reellement preparee est livree
  const shippedQty = (l) => l.unavailable ? l.qty_picked : (l.qty_picked > 0 ? l.qty_picked : l.qty);
  const rows = lines.filter((l) => shippedQty(l) > 0).map((l) =>
    `<tr><td>${esc(l.isbn)}</td><td>${esc(l.title)}</td><td class="num">${shippedQty(l)}</td></tr>`).join('');
  const total = lines.reduce((s, l) => s + shippedQty(l), 0);
...
/* --------- Etiquettes code-barres des caisses & chariots (Code 39) --------- */
route('GET', '/print/labels/crates', async (ctx) => {
  const ids = ctx.query.ids ? String(ctx.query.ids).split(',').map(Number) : null;
  let crates = db.prepare('SELECT * FROM crates WHERE active = 1 ORDER BY type, code').all();
  if (ids) crates = crates.filter((c) => ids.includes(c.id));
  if (!crates.length) throw new ApiError(404, 'Aucune caisse');
  const labels = crates.map((c) => `<div class="label">
      ${code39Svg(c.code)}
      <div class="lcode">${esc(c.code)}</div>
      <div class="letage">${c.type === 'chariot' ? 'Chariot' : 'Caisse'} de picking</div>
    </div>`).join('');
  const html = `<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><title>Etiquettes caisses &amp; chariots</title>
<style>
  body { font-family: 'Segoe UI', Arial, sans-serif; margin: 0; padding: 20px; }
  .sheet { display: grid; grid-template-columns: repeat(2, 1fr); gap: 14px; }
  .label { border: 1px dashed #94a3b8; border-radius: 8px; padding: 16px 12px; text-align: center; page-break-inside: avoid; }
