...
/** Quantite totale achetee par un client pour un produit, toutes factures validees confondues. */
function purchasedQty(clientId, productId) {
  return db.prepare(`SELECT COALESCE(SUM(il.qty),0) AS s FROM invoices i
    JOIN invoice_lines il ON il.fk_invoice = i.id
    WHERE i.fk_client = ? AND il.fk_product = ? AND i.type = 'facture' AND i.status >= 1`)
    .get(clientId, productId).s;
}

/**
 * Quantite deja acceptee en retour pour ce client/produit, tous retours confondus
 * (en cours de scan + finalises). excludeLineId permet d'exclure la ligne en cours
 * d'edition pour ne pas se compter deux fois.
 */
function acceptedReturnQty(clientId, productId, excludeLineId) {
  let sql = `SELECT COALESCE(SUM(rl.qty),0) AS s FROM return_lines rl
    JOIN returns r ON r.id = rl.fk_return
    WHERE r.fk_client = ? AND rl.fk_product = ? AND rl.line_status = 1`;
  const args = [clientId, productId];
  if (excludeLineId) { sql += ' AND rl.id != ?'; args.push(excludeLineId); }
...
module.exports = {
  round2, createInvoice, stockMove, gisementMove, findByIsbn, lastSaleDate, purchasedQty, acceptedReturnQty,
