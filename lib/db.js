...
CREATE TABLE IF NOT EXISTS purchase_orders (
  ...
  ref TEXT NOT NULL UNIQUE,
  fk_supplier INTEGER NOT NULL REFERENCES thirdparties(id),
  status INTEGER NOT NULL DEFAULT 0, -- 0 brouillon, 1 envoyee, 2 partiellement recue, 3 recue, -1 annulee
  date_order TEXT NOT NULL DEFAULT (date('now')),
  note TEXT,
  date_creation TEXT NOT NULL DEFAULT (datetime('now')),
  fk_user_creat INTEGER
...
CREATE TABLE IF NOT EXISTS purchase_order_lines (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  fk_po INTEGER NOT NULL REFERENCES purchase_orders(id) ON DELETE CASCADE,
  fk_product INTEGER NOT NULL REFERENCES products(id),
  qty REAL NOT NULL DEFAULT 1,
  qty_received REAL NOT NULL DEFAULT 0,
  buy_price_ht REAL NOT NULL DEFAULT 0,
  position INTEGER DEFAULT 0
);
...
CREATE TABLE IF NOT EXISTS receptions (
  ...
  ref TEXT NOT NULL UNIQUE,
  fk_supplier INTEGER NOT NULL REFERENCES thirdparties(id),
  fk_po INTEGER REFERENCES purchase_orders(id),
  status INTEGER NOT NULL DEFAULT 0, -- 0 en cours de scan, 1 validee (stock entre)
  note TEXT,
  date_creation TEXT NOT NULL DEFAULT (datetime('now')),
  fk_user_creat INTEGER
);
...
CREATE TABLE IF NOT EXISTS reception_lines (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  fk_reception INTEGER NOT NULL REFERENCES receptions(id) ON DELETE CASCADE,
  fk_product INTEGER NOT NULL REFERENCES products(id),
  qty REAL NOT NULL DEFAULT 1,
  buy_price_ht REAL
);
...
CREATE TABLE IF NOT EXISTS shipments (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  ref TEXT NOT NULL UNIQUE,
  fk_order INTEGER NOT NULL REFERENCES orders(id),
  fk_client INTEGER NOT NULL REFERENCES thirdparties(id),
  carrier TEXT,
  tracking TEXT,
  ...
  weight_kg REAL,
  note TEXT,
  date_shipment TEXT NOT NULL DEFAULT (date('now')),
  fk_user_creat INTEGER
);

CREATE TABLE IF NOT EXISTS crates (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  code TEXT NOT NULL UNIQUE,           -- le code sert de code-barres (Code 39)
  type TEXT NOT NULL DEFAULT 'caisse', -- caisse | chariot
  fk_order INTEGER REFERENCES orders(id), -- commande en cours de preparation / d'emballage
  active INTEGER NOT NULL DEFAULT 1,
  date_creation TEXT NOT NULL DEFAULT (datetime('now'))
);
...
CREATE TABLE IF NOT EXISTS inventories (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  ref TEXT NOT NULL UNIQUE,
  fk_gisement INTEGER NOT NULL REFERENCES gisements(id),
  status INTEGER NOT NULL DEFAULT 0, -- 0 en cours, 1 valide (ajustements appliques)
  note TEXT,
  date_creation TEXT NOT NULL DEFAULT (datetime('now')),
  fk_user_creat INTEGER
);
...
CREATE TABLE IF NOT EXISTS inventory_lines (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  fk_inventory INTEGER NOT NULL REFERENCES inventories(id) ON DELETE CASCADE,
  fk_product INTEGER NOT NULL REFERENCES products(id),
  qty_counted REAL NOT NULL DEFAULT 0,
  qty_theoretical REAL NOT NULL DEFAULT 0
);

CREATE INDEX IF NOT EXISTS idx_polines_po ON purchase_order_lines(fk_po);
CREATE INDEX IF NOT EXISTS idx_rclines_reception ON reception_lines(fk_reception);
CREATE INDEX IF NOT EXISTS idx_shipments_order ON shipments(fk_order);
CREATE INDEX IF NOT EXISTS idx_invlines_inventory ON inventory_lines(fk_inventory);
...
CREATE INDEX IF NOT EXISTS idx_crates_order ON crates(fk_order);
`);

/* --------------------------------------------------- migrations (bases existantes) */
try { db.exec('ALTER TABLE invoices ADD COLUMN date_due TEXT'); } catch { /* colonne deja presente */ }
try { db.exec('ALTER TABLE orders ADD COLUMN fk_backorder INTEGER'); } catch { /* deja presente */ }
try { db.exec('ALTER TABLE products ADD COLUMN stock_min REAL DEFAULT 0'); } catch { /* deja presente */ }
try { db.exec('ALTER TABLE thirdparties ADD COLUMN siret TEXT'); } catch { /* deja presente */ }
try { db.exec('ALTER TABLE thirdparties ADD COLUMN discount_pct REAL DEFAULT 0'); } catch { /* deja presente */ }
try { db.exec('ALTER TABLE thirdparties ADD COLUMN credit_limit REAL DEFAULT 0'); } catch { /* deja presente */ }
try { db.exec('ALTER TABLE thirdparties ADD COLUMN payment_terms_days INTEGER'); } catch { /* deja presente */ }
try { db.exec('ALTER TABLE order_lines ADD COLUMN unavailable INTEGER NOT NULL DEFAULT 0'); } catch { /* deja presente */ }
...
function seed() {
  ...
  // Caisses et chariots de picking (le code sert de code-barres)
  const insCrate = db.prepare('INSERT INTO crates (code, type) VALUES (?,?)');
  insCrate.run('CAISSE-01', 'caisse');
  insCrate.run('CAISSE-02', 'caisse');
  insCrate.run('CAISSE-03', 'caisse');
  insCrate.run('CHARIOT-01', 'chariot');

  // Seuils de reassort de demonstration
  db.prepare('UPDATE products SET stock_min = 30 WHERE id IN (4, 6)').run();
  db.prepare('UPDATE products SET stock_min = 50 WHERE id = 3').run();
